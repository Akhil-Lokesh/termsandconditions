...
class StructureExtractor:
    ...
    async def extract_structure(self, text: str) -> Dict[str, Any]:
        ...
        for pattern_idx, pattern in enumerate(self.SECTION_PATTERNS):
            ...
            if sections and len(sections) >= 3:
                if self.debug:
                    logger.info(
                        f"✓ Pattern {pattern_idx + 1} matched: {len(sections)} sections"
                    )
                    logger.info(f"  Pattern: {pattern}")
                break
            elif sections and self.debug:
                logger.debug(
                    f"✗ Pattern {pattern_idx + 1}: Only {len(sections)} sections (need >=3)"
                )
        ...
        for section in sections:
            ...
            if self.debug and len(clauses) > 0:
                logger.debug(
                    f"Section '{section['title'][:30]}...': {len(clauses)} clauses"
                )

        if self.debug:
            logger.info(
                f"Final extraction: {len(sections)} sections, {total_clauses} clauses"
            )
    ...
    async def _extract_clauses(self, section_text: str) -> List[Dict[str, Any]]:
        ...
        for pattern in self.CLAUSE_PATTERNS:
            found_clauses = await self._extract_clauses_with_pattern(
        ...
        for pattern in self.BULLET_PATTERNS:
            ...
            if found_clauses and len(found_clauses) >= 2:
                if self.debug:
                    logger.debug(
                        f"  Bullet pattern matched: {pattern} ({len(found_clauses)} clauses)"
                    )
        ...
        stats = {
            **result,
            "stats": {
                ...
                "avg_clauses_per_section": (
                    sum(clause_counts) / len(clause_counts) if clause_counts else 0
                ),
                "min_clauses": min(clause_counts) if clause_counts else 0,
                "max_clauses": max(clause_counts) if clause_counts else 0,
                "sections_with_multiple_clauses": sum(
                    1 for c in clause_counts if c > 1
                ),
            },
        }
